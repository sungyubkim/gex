...
def get_posterior_full(trainer, dataset_opt, *args, **kwargs):
    ...
    hess_fn_p = jax.pmap(jax.jacrev(loss_fn))
    ...
    print(f'Decompose Jacobian')
    _, S, Ut = np.linalg.svd(hess, full_matrices=False)
    U = Ut.T
    # L = np.zeros(U.shape[0])
    # L[:len(S)] = S**2
    L = S**2
    U = U[:,-50:]
    L = L[-50:]
    ...
    # L = L[:-10]
    # U = U[:,:-10]
    # hess_inv = U @ np.diag((L + 100)**(-1)) @ U.T # (P, P)
    ...
    # pert = U @ np.diag((L - np.min(L) + 1e-8)**(-1)) @ (U.T @ pert) # (P, M)
    # return pert
