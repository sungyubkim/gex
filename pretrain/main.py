...
from gex.models import resnet, vit, mixer, convnext, vgg, swem
...
def init_trainer(rng, batch, num_classes, num_train):
    ...
    model_settings = FLAGS.model.split('_')
    ...
    elif 'vit' in model_settings[0]:
        model, model_size, patch_size = model_settings
        patch_size = int(patch_size)
        model_method = getattr(vit, f'ViT_{model_size}')
        net = partial(
            model_method,
            name=FLAGS.model,
    ...
    elif 'mixer' in model_settings[0]:
        model, model_size, patch_size = model_settings
        patch_size = int(patch_size)
        model_method = getattr(mixer, f'Mixer_{model_size}')
        net = partial(
            model_method,
            name=FLAGS.model,
            patch_size=patch_size,
    ...
    elif 'convnext' in model_settings[0]:
        model, model_size = model_settings
        model_method = getattr(convnext, f'ConvNext_{model_size}')
        net = partial(
            model_method,
            name=FLAGS.model,
            num_classes=num_classes,
        )
    elif 'resnet' in model_settings[0]:
        model, model_size = model_settings
        model_method = getattr(resnet, f'ResNet{model_size}')
        net = partial(
            model_method,
            name=FLAGS.model,
            num_classes=num_classes,
            imagenet=(FLAGS.dataset=='imagenet2012'),
        )
    elif 'vgg' in model_settings[0]:
        model_method = getattr(vgg, 'VGG')
        net = partial(
            model_method,
            name=FLAGS.model,
            num_classes=num_classes,
...
def create_lr_sched(num_train):
    ...
    warmup_step = FLAGS.warmup_ratio * total_step
...
def main(_):
    ...
    step_per_epoch = np.ceil(float(ds.num_train) / FLAGS.batch_size_total).astype(int)
    update_freq = FLAGS.batch_size_total // FLAGS.batch_size_device
    step_per_epoch = step_per_epoch * update_freq
    
    save_epoch = int(0.5 * FLAGS.num_epochs)
    save_freq = (FLAGS.num_epochs - save_epoch) // FLAGS.num_ckpt
    ...
    if not(FLAGS.eval):
        ...
        for epoch in pbar:
            ...
            for _ in range(step_per_epoch):
                ...
                res_list.append(log)
                pbar.set_postfix(ckpt.pretty_log({'epoch': epoch, 'loss_sgd': log['loss_sgd'], 'acc_sgd': log['acc_sgd']}))
